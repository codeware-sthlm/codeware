 ...
 * _Special case_
 *
 * For **development**, the host is read from the `Tenant-Host` header instead.
 *
...
export const resolveTenant = async (args: {
  ...
  // Guard 'tenant-host' to development only!
  const host =
    (env.DEPLOY_ENV === 'development' ? headers['tenant-host'] : '') ||
    headers['host'];
