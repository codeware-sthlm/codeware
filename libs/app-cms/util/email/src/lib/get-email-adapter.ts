...
export const getEmailAdapter = (env: Env) => {
  ...
  if (env.EMAIL?.smtp) {
    const { defaultFromAddress, defaultFromName, host, port } = env.EMAIL.smtp;
  ...
  // than a decision, and a dropped confirmation is hard to notice — so make an
  // inbox on demand instead of going quiet.
