 ...
 *
 * @param remoteDataUrl Remote URL is **DISABLED**, local filesystem is used.
 * @returns Base seed data for the application.
 */
export const seedData = (remoteDataUrl: string | undefined) => {
  if (remoteDataUrl) {
    console.warn(
      'Remote URL is not used to seed files in development:',
      remoteDataUrl
    );
  }

  // Resolve all seed files from `media` folder
